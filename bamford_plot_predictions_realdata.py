...
if __name__ == '__main__':
    ...
    test_data = pd.read_csv('output_csvs/real_galaxies_run_final_method.csv')
    test_data['elpetro_mass'] = np.log10(test_data['elpetro_mass'])
    assert len(test_data) > 0

    logging.info('Extracting test sample')
    # Remove the test sample

    unique_gal_names=pd.unique(test_data['iauname'])
    ...
    for test_sample_names in np.array_split(pd.unique(test_data['iauname'])[gal_min:gal_max], max_batches): #batches the data into [max_batches] 

        assert len(test_sample_names) > 0

        test_sample = test_data[test_data['iauname'].isin(test_sample_names)].reset_index(drop=True)

        full_data_sample = full_data.reset_index(drop=True)
        ...
        for test_name in test_sample_names:
            ...
            for i in range(len(test_galaxy.index)):
                ...
                weighted_means_list_smooth.append(prediction_smooth) #not actually a weighted means list, uses sampling
    ...
    df_cumulative.to_csv('output_csvs/real_gals_bamford_predictions_{0}.csv'.format(args.batch_gal_min), header=df_cumulative.columns, index=False)